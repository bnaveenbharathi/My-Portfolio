...
},
{
      ...
      description: [
        ...
        'Received mentorship from Mohamed Ijass, gaining valuable insights into frontend best practices, code optimization, and professional development methodologies.'
      ]
    },
{
...
,
{
  ...
  return (
    <div className='experience-section relative min-h-screen flex items-center justify-center py-20 px-6 overflow-hidden'>
      
      <div className="absolute top-20 left-10 w-24 h-24 border-4 border-gray-600/20 rotate-45 animate-spin-slow"></div>
      <div className="absolute bottom-10 right-20 w-32 h-32 bg-gray-600/10 rounded-full animate-pulse"></div>
      <div className="absolute top-1/3 right-10 w-16 h-16 border-4 border-gray-500/20 rounded-full animate-bounce-slow"></div>
              ...
              >
                ...
                <div className={`ml-20 md:ml-0 ${index % 2 === 0 ? 'md:pr-[calc(50%+2rem)]' : 'md:pl-[calc(50%+2rem)]'}`}>
                      ...
                      <div className="relative z-10">
                        ...
                        <div className="flex items-start justify-between gap-4 mb-3">
                          <div className="flex-1">
                            <h3 className="text-xl md:text-2xl font-bold text-gray-200 poppins group-hover:text-gray-100 transition-colors">
                              {exp.role}
                            </h3>
                            <p className={`text-lg font-semibold mt-1 bg-gradient-to-r ${exp.color} bg-clip-text text-transparent`}>
                              {exp.company}
                            </p>
                          </div>
                        ...
                        <div className="flex flex-wrap gap-4 text-sm text-gray-400 montserrat">
                        ...
                        <div className="flex flex-wrap gap-2 mt-4">
                          {exp.technologies.slice(0, 4).map((tech, techIndex) => (
                            <span
                              key={techIndex}
                              className="px-3 py-1 bg-gray-700/50 border border-gray-600/50 rounded-full text-xs text-gray-300 poppins"
                            >
                              {tech}
                            </span>
                          ))}
                          {exp.technologies.length > 4 && (
                            <span className="px-3 py-1 bg-gray-700/30 border border-gray-600/30 rounded-full text-xs text-gray-400 poppins">
                      ...
                      <div className="px-6 pb-6 border-t border-gray-700/50 pt-6">
                        <div className="space-y-3">
                          {exp.description.map((desc, descIndex) => (
                            <div key={descIndex} className="flex gap-3 animate-slide-up" style={{ animationDelay: `${descIndex * 50}ms` }}>
                              <div className={`mt-2 w-1.5 h-1.5 rounded-full bg-gradient-to-r ${exp.color} flex-shrink-0`}></div>
                              <p className="text-gray-400 montserrat text-sm leading-relaxed">
                                {desc}
                              </p>
                            </div>
                          ))}
                          ...
                          <div className="mt-6 pt-6 border-t border-gray-700/30">
                            ...
                            <div className="flex flex-wrap gap-2">
                              {exp.technologies.map((tech, techIndex) => (
                                <span
                                  key={techIndex}
                                  className="px-3 py-1 bg-gray-700/50 border border-gray-600/50 rounded-full text-xs text-gray-300 poppins hover:bg-gray-700/70 transition-colors"
                                >
                                  {tech}
                                </span>
                              ))}
                  ...
                  <div className={`hidden md:block absolute top-0 ${index % 2 === 0 ? 'left-[calc(50%+2rem)]' : 'right-[calc(50%+2rem)]'}`}>
                    <div className="bg-gray-800/60 backdrop-blur-sm border border-gray-600/50 rounded-lg px-3 py-1 text-xl text-gray-300 poppins font-semibold">
